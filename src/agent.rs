...
impl AgentLoop {
    pub async fn new(config: AgentConfig) -> Result<Self> {
        ...
        let context = Arc::new(RwLock::new(ContextEngine::new(&config.context_path).await?));
        let eval = Arc::new(EvalEngine::new().await?);
        let git = Arc::new(GitOps::new(&config.workspace_path).await?);
        let harness = Arc::new(ClaudeHarness::new(config.claude_api_key.clone()).await?);
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        let branch_name = format!("autoclaw-{}", &Uuid::new_v4().to_string()[..8]);
        ...
        for iteration in start_iteration..self.config.max_iterations {
            if self.stop_requested.load(Ordering::SeqCst) {
                info!("Stop requested; halting loop at iteration {}", iteration);
                break;
            }
                    ...
                    if is_improvement {
                        ...
                        *best = Some(result.clone());
                        ...
                        if self.config.auto_commit {
                            ...
                            self.update_experiment_commit(&experiment.id, commit_hash)
                                .await?;
                        }

                        self.tx
                            .send(AgentEvent::ImprovementFound(experiment.clone()))
                            .await?;
                        ...
                        self.tx
                            .send(AgentEvent::RegressionDetected(experiment.clone()))
                            .await?;
                        // Revert changes
    ...
    async fn run_experiment(&self, iteration: usize) -> Result<Experiment> {
        ...
        self.tx
            .send(AgentEvent::ExperimentStarted(experiment_id.clone()))
            .await?;
        ...
        let hypothesis = self
            .harness
            .generate_hypothesis(&context_str, iteration)
            .await?;

        // Generate code changes
        let code_changes = self
            .harness
            .generate_changes(&context_str, &hypothesis)
            .await?;
        ...
        let _ = self
            .metrics
            .record_experiment(crate::metrics::ExperimentMetrics {
                iteration,
                duration_ms,
                score: experiment.result.as_ref().map(|r| r.score).unwrap_or(0.0),
                passed: experiment
                    .result
                    .as_ref()
                    .map(|r| r.passed)
                    .unwrap_or(false),
                tokens_used: 0,
                code_changes: experiment.code_changes.len(),
            })
            .await;

        self.experiments.write().await.push(experiment.clone());
        self.tx
            .send(AgentEvent::ExperimentCompleted(experiment.clone()))
            .await?;
    ...
    async fn update_experiment_commit(
        &self,
        experiment_id: &str,
        commit_hash: String,
    ) -> Result<()> {
    ...
    /// Receive the next agent event, if any is queued.
    pub async fn next_event(&self) -> Option<AgentEvent> {
        self.rx.write().await.try_recv().ok()
    }
}
